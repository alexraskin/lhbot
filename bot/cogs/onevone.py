...
class OneVOne(commands.Cog, name="OneVOne"):
    ...
    async def one_v_one(
        ...
        embed.add_field(
            name=f"{str(ctx.author).strip('#')} is playing",
            value=f"{hero_one} with {hero_one_health} health",
        )
        embed.add_field(
            name=f"{user} is playing", value=f"{hero_two} with {hero_two_health} health"
        )
        ...
        while hero_one_health > 0 and hero_two_health > 0:
            ...
            second_embed.add_field(
                name=f"{ctx.author}",
                value=f"Playing {hero_one} with {hero_one_health} health",
            )
            second_embed.add_field(
                name=f"{user}",
                value=f"Playing {hero_two} with {hero_two_health} health",
            )
